---
name: colab-mcp
description: Interactively author, edit, and run cells in a Google Colab notebook through the colab-mcp browser bridge — using Google's compute while the user watches in their Chrome tab. Invoke when the user wants Claude to write, run, or edit cells in a live Colab session (scratch or an existing Drive notebook). For debugging errors *inside* a Colab runtime (segfaults, install failures, kernel crashes), use the `colab-debug` skill instead.
---
...
4. If `false`: do NOT retry in the same session. Have the user restart Claude Code.
