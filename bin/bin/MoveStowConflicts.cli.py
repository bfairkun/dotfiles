...
def cmdline_args(Args=None):
    ...
    p = argparse.ArgumentParser(
        description=__doc__, formatter_class=CustomFormatter, add_help=True
    )
    ...
    subparsers = p.add_subparsers(
        title="subcommands",
        help="Choose a subcommand and run with -h for more help. Subcommands listed below:",
        dest="sub_command",
    )
    # create the parser_a for the first command
    parser_a = subparsers.add_parser(
        "MoveStowConflictFilesToDir",
        add_help=False,
        parents=[parent],
        formatter_class=CustomFormatter,
        help="Find which files in subtrees of source dir (i.e. a stow dir) would conflict if they were to be moved (or symlink 'stowed') into a target dir. Move those files into a NewDir to allow stowing without conflicts",
    )
    parser_a.add_argument(
        "--SourceDir",
        help="Source stow directory. In the example use case, this would be the dotfiles stow directory",
        metavar="<Path/To/Directory>",
        default="./",
    )
    parser_a.add_argument(
        "--TargetDir",
        help="Target stow directory. In the example use case, this would be the home directory where you want to later create symlinks in place of the existing dotfiles",
        metavar="<Path/To/Directory>",
        default="../",
    )
    parser_a.add_argument(
        "--NewDir",
        help="New directory. In the example use case, this would be something like Dir/To/Keep/OldDotfiles",
        metavar="<Path/To/Directory>",
        default="PreStowConflictFiles/",
    )
    parser_a.add_argument(
        "--SubtreeDirs",
        help="""Sub directory(s) in source directory to check for stow conflicts. This is equivalent to package directories for stow. In the example use case, this would be something like git vim. Glob pattern expansions, are acceptable """,
        metavar="<SubDirInSourceDirectory>",
        nargs="+",
        required=True,
    )
    # create the parser_b for the next command
    parser_b = subparsers.add_parser(
        "MoveFilesToDirSafely",
        parents=[parent],
        add_help=False,
        formatter_class=CustomFormatter,
        help="Move files in source dir into a target dir",
    )
    parser_b.add_argument(
        "--TargetDir",
        help="Target directory where you want files to be moved to. In the example use case, this would be your home directory",
        metavar="<Path/To/Directory>",
        required=True,
    )
    parser_b.add_argument(
        "--SourceDir",
        help="Source directory with files you want to move. In the example use case, this would be the directory where you put all your old dotfiles and now you want to move those same files back to home",
        metavar="<Path/To/Directory>",
        required=True,
    )
...
def MoveStowConflictFilesToDir(
    TargetDir="../",
    SourceDir="./",
    SubtreeDirs=[""],
    NewDir="PreStowConflictFiles",
    dryrun=False,
    **kwargs
):
            ...
            if not os.path.islink(target_fn) and os.path.exists(target_fn):
                ...
                if not dryrun:
                    if not os.path.exists(New_fn):
                        ...
                        logging.warning(
                            "    ...Failure. {} already exists. Did not overwrite.".format(
                                New_fn
                            )
                        )
