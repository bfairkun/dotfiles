...
parser.add_argument(
    ...
    "--FileIn",
    default="-",
    help="REQUIRED. Stranded 6 column bed file of introns that need alternative splice site classification. Use 'stdin' or '-' to read from stdin",
    required=True,
)
parser.add_argument(
    "-A",
    "--AnnotationsBed",
    help="REQUIRED. Stranded 6 column bed file of annotated introns",
    required=True,
)
parser.add_argument(
    "-O",
    "--FileOut",
    default="stdout",
    help="REQUIRED. Output file. Same as input file but with extra field of alternative splice site classification field. Use 'stdout' to write to stdout",
    required=True,
)
parser.add_argument(
    "--quiet", help="OPTIONAL. quiet the output verbosity", action="store_true"
)
...
def GetSpliceDonorAndAcceptorCoordinates(chrom, start, stop, strand):
    ...
    if strand == "+":
        ...
        print(
            "Failed... Need strand information coded as + or -. This information should be stored in field6 of bed file"
        )
...
for line in fhIn:
    ...
    if (Donor, Acceptor) in AnnotatedSplicePairs:
        ...
        Donor not in AnnotatedSpliceDonors and Acceptor not in AnnotatedSpliceAcceptors
    ):
        AS_type = "NewIntron"
